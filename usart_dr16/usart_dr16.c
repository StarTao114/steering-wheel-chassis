...
void RC_Init(uint8_t *rx1_buf, uint8_t *rx2_buf, uint16_t dma_buf_num)//数组0 1   缓存长度
{
    ...
    RCC_APB1PeriphResetCmd(RCC_APB1Periph_UART4, ENABLE);//RCC_APB1PeriphResetCmd(RCC_APB1Periph_USART3, DISABLE);
	  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_DMA1,ENABLE);

    GPIO_PinAFConfig(GPIOC, GPIO_PinSource11, GPIO_AF_UART4); //PC11  usart4 rx
    /* -------------- Configure GPIO ---------------------------------------*/
    {
    ...
    /* -------------- Configure DMA -----------------------------------------*/
    {
        ...
        DMA_DoubleBufferModeConfig(DMA1_Stream2, (uint32_t)rx2_buf, DMA_Memory_0);
        DMA_DoubleBufferModeCmd(DMA1_Stream2, ENABLE);
        DMA_Cmd(DMA1_Stream2, DISABLE); //Add a disable  DMA_Cmd(DMA1_Stream1, DISABLE)
        DMA_Cmd(DMA1_Stream2, ENABLE);//   DMA_Cmd(DMA1_Stream1, ENABLE)
    }
}
void RC_unable(void)
{

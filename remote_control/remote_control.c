...
{
    if (USART_GetITStatus(UART4, USART_IT_RXNE) != RESET)//检查指定的USART中断是否发生  接受中断
    {
        USART_ReceiveData(UART4);//
    }
    else if (USART_GetITStatus(UART4, USART_IT_IDLE) != RESET)//空闲中断
    {
        ...
        if(DMA_GetCurrentMemoryTarget(DMA1_Stream2) == 0)
        {
            ...
            //清DMA中断标志
            DMA_ClearFlag(DMA1_Stream2, DMA_FLAG_TCIF4 | DMA_FLAG_HTIF4);
